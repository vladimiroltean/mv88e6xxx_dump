...
#define MAX_SNAPSHOT_DATA		100 * 1024
...
static void delete_snapshot_port_id(struct sja1105_ctx *ctx,
...
static void delete_snapshot(struct sja1105_ctx *ctx, struct nlattr **tb)
{
...
static void sja1105_dump_static_config(struct sja1105_ctx *ctx)
{
	struct sja1105_static_config config;
	sja1105_config_valid_t valid;
	enum sja1105_blk_idx blk_idx;

	valid = sja1105_static_config_unpack(ctx->snapshot_data, ctx->data_len,
					     &config);
	if (valid != SJA1105_CONFIG_OK) {
		puts(sja1105_static_config_error_msg[valid]);
		return;
	}
...
static void cmd_static_config(struct sja1105_ctx *ctx)
{
	...
	return sja1105_dump_static_config(ctx);
}
